...
class TacTu:
    def __init__(self, moitruong: MoiTruong):
        ...
        self._thoidiemvatphamtontaihanhtrang_map = {}
        self._trihoansudungvatpham_map = {}
    ...
    def them_tennhanvattancong_theotennhanvat(self, tennhanvats):
        if not tennhanvats:
            return

        tennhanvats = [ten.strip() for ten in tennhanvats.split(",") if ten.strip()]

        soluongthemmoi = 0
        for tennhanvat in tennhanvats:
            if tennhanvat not in self._tennhanvattancongs:
                self._tennhanvattancongs.add(tennhanvat)
                soluongthemmoi += 1

        if soluongthemmoi > 0:
            print(f"Danh sách nhân vật tấn công cập nhật từ UI: {self._tennhanvattancongs}")
            phatam(f"Đã thêm {soluongthemmoi} mục tiêu từ giao diện")
    ...
    def action_test(self):
        ...
        for i in range(3000):
            iddiachikynang = self.moitruong.get_iddiachikynang(i)
            if iddiachikynang < 0:
                continue
            print("{}: {}".format(i, iddiachikynang))
    ...
    def _kiemtrathoamandieukientancong(self, idnhanvat):
        ...
        if self.moitruong.get_is_anthan(idnhanvat):
            return False
    ...
    def action_tudongsudungvatpham(self):
        ...
        if diempk > 0:
            if diempk >= 8 or (diempk >= 5 and phantramsinhluchientai <= 70) or (diempk >= 3 and phantramsinhluchientai <= 60) or phantramsinhluchientai <= 50 or self._phantramsinhlucmatdi >= 20:
                ...
                if sothutuvatpham != -1:
                    thoidiemcoquanamthuy = self._thoidiemvatphamtontaihanhtrang_map.get(QUANAMTHUY, 0)

                    if thoidiemcoquanamthuy == 0:
                        self._thoidiemvatphamtontaihanhtrang_map[QUANAMTHUY] = time.time()

                        if phantramsinhluchientai <= 30 or self._phantramsinhlucmatdi >= 20:
                            self._trihoansudungvatpham_map[QUANAMTHUY] = random.uniform(0.1, 0.4)
                        elif 30 < phantramsinhluchientai <= 60:
                            self._trihoansudungvatpham_map[QUANAMTHUY] = random.uniform(0.8, 1.5)
                        else:
                            self._trihoansudungvatpham_map[QUANAMTHUY] = random.uniform(1.5, 3.0)
                        return

                    thoigiandatroiqua = time.time() - thoidiemcoquanamthuy
                    trihoanthucte = self._trihoansudungvatpham_map.get(QUANAMTHUY, 1.25)

                    if (phantramsinhluchientai <= 30 or self._phantramsinhlucmatdi >= 20) and trihoanthucte > 0.4:
                        trihoanthucte = random.uniform(0.1, 0.4)
                        self._trihoansudungvatpham_map[QUANAMTHUY] = trihoanthucte

                    if thoigiandatroiqua >= trihoanthucte:
                        if self.action_sudungvatpham(QUANAMTHUY, delay = 0):
                            self._thoidiemvatphamtontaihanhtrang_map[QUANAMTHUY] = 0
                            return
                else:
                    self._thoidiemvatphamtontaihanhtrang_map[QUANAMTHUY] = 0
        ...
        for tenvatpham in [THANCAUPHU, THANTIENTAN, NIETBANCHU, ]:
            ...
            if sothutuvatpham != -1:
                thoidiemcovatpham = self._thoidiemvatphamtontaihanhtrang_map.get(tenvatpham, 0)

                if thoidiemcovatpham == 0:
                    self._thoidiemvatphamtontaihanhtrang_map[tenvatpham] = time.time()
                    continue

                if (time.time() - thoidiemcovatpham) < 2.5:
                    continue

                if self.action_sudungvatpham(tenvatpham, delay = random.uniform(2, 4)):
                    self._thoidiemvatphamtontaihanhtrang_map[tenvatpham] = 0
                    return
            else:
                self._thoidiemvatphamtontaihanhtrang_map[tenvatpham] = 0
    ...
    def action_xulydichuyenuutien(self):
        ...
        if yeucauduocchon:
            ...
            if loaidichuyen == "dichuyengiukhoangcachtoithieu" or (yeucauduocchon == self._yeucaudichuyentheosautruongnhom and is_quaxatruongnhom) or (self.moitruong.get_idmaupk() == IDMAUPK_DO and not self.get_is_dusoluongtoithieu(QUANAMTHUY, 1)):
            ...
            if self.moitruong.get_idmaupk() == IDMAUPK_DO and not self.get_is_dusoluongtoithieu(QUANAMTHUY, 1):
                self.moitruong.set_is_tamngungtancong(True)
            else:
                self.moitruong.set_is_tamngungtancong(False)
    ...
    def _get_is_dichuyenduongthang(self, idmuctieu, saisogoc = 15):
        ...
        x2, y2 = self.moitruong.get_toado(idmuctieu)
        x2_saptoi, y2_saptoi = self.moitruong.get_toadosaptoi(idmuctieu)

        if x2_saptoi <= 0 or (x2 == x2_saptoi and y2 == y2_saptoi):
            return True
        ...
        mag_vt = math.sqrt(vt_x ** 2 + vt_y ** 2)
        mag_vm = math.sqrt(vm_x ** 2 + vm_y ** 2)

        if mag_vt == 0 or mag_vm == 0:
            return True

        dot_product = vt_x * vm_x + vt_y * vm_y
        cos_theta = dot_product / (mag_vt * mag_vm)

        cos_theta = max(min(cos_theta, 1.0), -1.0)

        angle_deg = math.degrees(math.acos(cos_theta))

        if angle_deg <= saisogoc or angle_deg >= (180 - saisogoc):
            return True
    ...
    def action_xulytancong(self):
        ...
        try:
            ...
            idkynangtaytrai = self.moitruong.get_idkynangtaytrai()
            if self.moitruong.get_idhephai() == IDHEPHAI_DINHAN:
                ...
                if is_cothesudungkynang:
                    ...
                    if idmuctieu > 0:
                        ...
                        if self.moitruong.get_idbandohientai() == IDBANDO_NGOCHUCUNG10NAMTRUOC:
                            self.moitruong.action_sudungkynangtoadochichuot(idkynangtaytrai, 0)
                    ...
                    if idmuctieu > 0:
                        ...
                        if self._is_giukhoangcach:
                            ...
                            if (is_nguoichoicangiukhoangcach or is_boss) and khoangcachmuctieu < khoangcachantoan:
                                ...
                                if not is_cokynangsansang or (self.moitruong.get_is_bidongbang(idmuctieu) and not self.moitruong.get_is_bidongbang()) or (idhephaimuctieu == IDHEPHAI_GIAPSI and self.moitruong.get_idtrangthainhanvat(idmuctieu) == IDTRANGTHAINHANVAT_TANCONG):
                                    yeucaudichuyenmoi = {
                        ...
                        if self.moitruong.get_idbandohientai() == IDBANDO_NGOCHUCUNG10NAMTRUOC:
                            if idkynangtaytrai == IDKYNANG_BANMONLONGPHU:
                            ...
                            elif idkynangtaytrai == IDKYNANG_THIETMABANGQUA:
                                self.moitruong.action_sudungkynangtoadochichuot(IDKYNANG_TINHTHONGBANGHE, random.randint(450, 475))
                            else:
                                self.moitruong.action_sudungkynangtoadochichuot(IDKYNANG_TINHTHONGHOAHE, random.randint(450, 475))
                        else:
                            if self.moitruong.get_is_kynangsansang(IDKYNANG_TAMMUOICHANHOA):
                ...
                khoangcachtancong = 600 if idkynangtaytrai in (IDKYNANG_XUYENTAMTIEN, IDKYNANG_THAUCOTTIEN) else 500
                is_cothesudungkynang = self.moitruong.get_is_dangbatauto() and self.moitruong.get_is_khuvuccothetancong() and self.moitruong.get_idtrangthaiclickchuot() != IDTRANGTHAICLICKCHUOT_CHUOTTRAI and not self.moitruong.get_is_dangtudongtimduong()
                if is_cothesudungkynang:
                    idmuctieu = self._idmuctieu if self._is_tudongtimkiemmuctieu else self.moitruong.get_idmuctieutancong()
                    if idmuctieu > 0:
                        ...
                        is_sudungbachboxuyenduong = self.moitruong.get_tennhanvat() != "OsamaDaEĐen"
                        if self._is_giukhoangcach:
                            ...
                            if (is_nguoichoicangiukhoangcach or is_boss) and khoangcachmuctieu < khoangcachantoan:
                                is_cokynangsansang = False
                                for idkynang in (IDKYNANG_BACHBOXUYENDUONG  if is_sudungbachboxuyenduong else IDKYNANG_BACHBOXUYENDUONGTATPHONGLV1, IDKYNANG_NHATNHANTHANHQUAN,):
                                    if self.moitruong.get_is_kynangsansang(idkynang):
                                        is_cokynangsansang = True
                                        break
                        ...
                        if self._is_danhchan and not self._is_duoitheo:
                            if 900 < khoangcachmuctieu <= 1200 and self.moitruong.get_is_kynangsansang(IDKYNANG_XASAT) and is_sudungxasat:
                                ...
                                self.moitruong.action_sudungkynangphudau(idmuctieu, IDKYNANG_BACHBOXUYENDUONG if is_sudungbachboxuyenduong else IDKYNANG_BACHBOXUYENDUONGTATPHONGLV1, random.randint(450, 475))
                        ...
                        if self._is_danhchan:
                            ...
                            if khoangcachtancong < khoangcachmuctieu < khoangcachphudau:
                                if 900 < khoangcachmuctieu <= 1200 and self.moitruong.get_is_kynangsansang(IDKYNANG_XASAT) and is_sudungxasat:
                                    self.moitruong.action_sudungkynangphudau(idmuctieu, IDKYNANG_XASAT, random.randint(550, 575))
                                    return
                                elif khoangcachmuctieu <= 1200 and self.moitruong.get_is_kynangsansang(IDKYNANG_BACHBOXUYENDUONG):
                                    self.moitruong.action_sudungkynangphudau(idmuctieu, IDKYNANG_BACHBOXUYENDUONG if is_sudungbachboxuyenduong else IDKYNANG_BACHBOXUYENDUONGTATPHONGLV1, random.randint(450, 475))
                                    return
                                elif 600 < khoangcachmuctieu <= 1200 and self.moitruong.get_is_kynangsansang(IDKYNANG_XUYENTAMTIEN):
                                    self.moitruong.action_sudungkynangphudau(idmuctieu, IDKYNANG_XUYENTAMTIEN, random.randint(550, 575))
                                    return

                        if khoangcachmuctieu > khoangcachtancong:
                            yeucaudichuyenmoi = {
                                "loaidichuyen": "dichuyengiukhoangcachtoida",
                                "idmuctieu": idmuctieu,
                                "khoangcach": khoangcachmuctieu - random.randint(100, 150) if is_muctieutiepcan else khoangcachmuctieu - random.randint(200, 300)
    ...
    def action_tudongphucsinh(self):
        ...
        if self.moitruong.get_is_nhanvatdachet() and self.moitruong.get_is_cohieuungbotro(IDHIEUUNGBOTRO_NIETBANCHU):
    ...
    def action_tudongmoitodoi(self):
        ...
        for tenungvien in self._tennhanvattodoitudongs:
            ...
            if time.time() - thoidiemgannhat > 2.5:
                idnhanvatungvien = self.action_timkiemnhanvat(tennhanvat = tenungvien, khoangcach = 1200)
                if idnhanvatungvien > 0:
                    dbidhientai = self.moitruong.get_dbidnhanvat(idnhanvatungvien)
                    if dbidhientai > 0:
                        ungviens.append((dbidhientai, tenungvien, thoidiemgannhat))
        if not ungviens:
            return
